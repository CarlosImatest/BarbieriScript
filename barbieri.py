'''
This barbieri works with:
    WINx64 Barbieri 4.7.19
    Spcelib.dll version: 4.45
    SpeclibDOC.dll version: 1.06
'''
...
class barbieri:
    ...
    def calibrate(self):
        ...
        # prompt user to insert film after calibration is done
        self.root.withdraw()
        self.root.attributes('-topmost', True) # bring the messagebox to the front so the user sees it right away
        ...
        self.root.attributes('-topmost', False) # reset the topmost attribute so future messageboxes don't always appear on top
